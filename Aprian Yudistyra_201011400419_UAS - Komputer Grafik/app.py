...
CROPPED_FOLDER = 'static/images/crop'
...
def crop(filename):
    ...
    cropped_filename = f"{file_name}_{position}{file_ext}"
    new_cropped_path = os.path.join(app.root_path, app.config['CROPPED_FOLDER'], cropped_filename)

    os.rename(cropped_path, new_cropped_path)

    return render_template('output.html', cropped_image=cropped_filename)
